...
import { CzyNaPewnoChceszOdwolacWizyteComponent } from './czy-na-pewno-chcesz-odwolac-wizyte/czy-na-pewno-chcesz-odwolac-wizyte.component';
...
export class OdwolanieWizytyComponent implements OnInit {
  ...
  ngOnInit() {
    this.listonosz
      .pobierz(Drzwi.informacjeOWizycieDoOdwolania + this.kodOdwolania)
      .then(k => {
        console.log(k);
  ...
  potwierdz() {
    ...
    okno.closed.pipe(take(1)).subscribe(k => {
      this.listonosz
        .usun(Drzwi.informacjeOWizycieDoOdwolania + this.wizyta.kodOdwolania)
        .then(k => {
    ...
    okno.dismissed.pipe(take(1)).subscribe(k => {
      this.przyciskDisabled = false;
    });
  }
